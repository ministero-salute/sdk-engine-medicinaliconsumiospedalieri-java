...
package it.mds.sdk.flusso.osp.service;
...
@Slf4j
@Component("flussoOspService")
public class FlussoOspService {
    ...
    private final  TracciatoSplitter<RecordDtoOsp> tracciatoSplitter;
    private final ConnettoreMds connettoreMds;
    private  final ConfigurazioneFlussoOsp conf;

  //  private final ParserTracciatoImpl parserTracciatoImpl;
  //  private final String CATEGORIA_FLUSSI = "OST";

    private final String NOME_FILE_XSD_OSP = "OSP.xsd";

    private final Configurazione config = new Configurazione();

    @Autowired
    public FlussoOspService(@Qualifier("parserRegoleOsp")  ParserRegole parserRegole,
                            @Qualifier("parserTracciato")  ParserTracciato parserTracciato,
                            @Qualifier("validationEngine")  ValidationEngine validationEngine,
                            @Qualifier("tracciatoSplitter")  TracciatoSplitter<RecordDtoOsp> tracciatoSplitter,
                            @Qualifier("connettoreMds")  ConnettoreMds connettoreMds,
                            @Qualifier("configurazioneFlussoOsp") ConfigurazioneFlussoOsp conf
                            //@Qualifier("parserTracciato") ParserTracciatoImpl parserTracciatoImpl
                            ) {
        ...
        this.conf = conf;
     //   this.parserTracciatoImpl = parserTracciatoImpl;
    }
    ...
    public void validazioneBlocchi(int dimensioneBlocco, String nomeFile, RegoleFlusso regoleFlusso, String idRun,
      ...
      //ParserTracciatoImpl parser = new ParserTracciatoImpl();
        ...
        do {
            ...
            List<RecordDtoOsp> recordConv =
                    bloccoValidazione.getRecordList().stream().map(k -> (RecordDtoOsp) k).collect(Collectors.toList());
...
//                TracciatoSplitterImpl tracciatoSplitterImpl = (TracciatoSplitterImpl) tracciatoSplitter;
//                dataroot = tracciatoSplitterImpl.creaDataroot(recordConv, dataroot);
            } else {
            ...
            if (isWriteOk) {
                ...
                URL urlXsd = this.getClass().getClassLoader().getResource(NOME_FILE_XSD_OSP);
        ...
        if (accettati > 0) {
            ...
            try {
                nomeFilePulito = validationEngine.puliziaFileDataroot(nomeFileXmlTmp, nomeFileXml, NOME_FILE_XSD_OSP);
